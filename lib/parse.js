...
module.exports = {
  ...
  extractData: function (xmlJsObj) {
    ...
    if (!data) {
      console.log('BIG ERROR WITH TIHS ONE!!')
      inspect(xmlJsObj)
      data = {}
      data.xml = ''
      return data
  ...
  buildItems: function (data) {
    ...
    data.itemIds.forEach((id) => {
      ...
      if (data.item852Data[id] && data.item876Data[id]) {
        ...
        // if we did not find a callnumber use the item
        if (!callnumber) {
          callnumber = data.item852Data[id]['3']
        }
  ...
  extract852: function (data) {
    data.items852.forEach((a852) => {
      ...
      if (itemNumber) {
        ...
        if (!cn) {
          cn = data.item852Data[itemNumber]['3']
        }
    ...
    if (!data.has001ControlNumber) {
      ...
      if (data.has001ControlNumber991y) {
        data.recordControlFields.push({
          _: `NYPL.b${data.has001ControlNumber991y}`,
          $: { tag: '001' }
        })
